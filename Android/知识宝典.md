...
### 十、AlertDialog,popupWindow,Activity区别。
...
### 十三、Parcelable和Serializable的区别和比较
...
### 十五、Requestlayout，onlayout，onDraw，DrawChild区别与联系。
...
**1、mAttachedScrap**：存放全部的ViewHolder对象。

**2、mChangedScrap**：存放发生改变的ViewHolder对象。

**3、mCachedViews**：存放remove调的视图的VIewHolder对象(保存的信息、绑定的数据未删除)，容量有限制默认是2。

**4、mRecyclerPool**：存放remove调的视图的VIewHolder对象（完全删除信息、数据等）。

### 十七、Looper中的Loop()方法不会造成anr
...
### 二十、事件分发中的onTouch 和onTouchEvent 有什么区别，又该如何使用？
...
### 二十一、setOnTouchListener与dispatchTouchEvent关系
...
### 二十三、击事件被拦截，但是想传到下面的View，如何操作？

​	requestDisallowInterceptTouchEvent()

### 二十四、网络请求缓存处理，okhttp如何处理网络缓存的？
...
​	给SplashActivity设置主题<item name="android:windowNoTitle">true</item> <item name="android:windowFullscreen">true</item> <item name="android:windowBackground">@drawable/layer_splash2</item>。主题设置背景图片，背景图片
...
​		网络、文件等流忘记关闭；手动注册广播时，退出时忘记 unregisterReceiver()；Service 执行完后忘记 stopSelf()；EventBus 等观察者模式的框架忘记手动解除注册；static 关键字修饰的成员变量；ListView 的 Item 泄露（初始化时缓存中没有View对象则convertView是null）；WebView造成的泄露（，应该调用它的destory()函数来销毁它）。二十七、rxjava 内存泄漏 （AutoDispose、RxLifecycle解决内存泄漏）
...
### 三十三、线程同步 synchronized 同步代码块 同步方法 同步锁。
...
​		1、为每个插件创建一个 Classloader（缺点是这种方案非常麻烦，为此要反射出一堆类型的对象 ，而且还要适配各种 Android版本 。）
...
​		1、预先占位先注册10个Services（当插件中的 Service 数量不可预知的时候，就束手无策了，比如手机助手、手机卫士这类 App 。）	
...
### 四十四、String为什么不可变，StringBuffer可变	
...
### 四十九、ArrayList、set、map每次扩大多少。

![img](C:\Users\Administrator\AppData\Local\YNote\data\qq432337FEDCB6EB5443353EB50AE37219\fa9d629cdc13481fa2198b258841da66\clipboard.png)
...
![img](C:\Users\Administrator\AppData\Local\YNote\data\qq432337FEDCB6EB5443353EB50AE37219\caa3a59492ab497f937de53a8001b18f\clipboard.png)
...
### 五十四、RecyclerView 如何添加头、脚以及间隔线。

![img](C:\Users\Administrator\AppData\Local\YNote\data\qq432337FEDCB6EB5443353EB50AE37219\e82a48a88742436ba984c1a3d665c516\clipboard.png)

![img](C:\Users\Administrator\AppData\Local\YNote\data\qq432337FEDCB6EB5443353EB50AE37219\91caccb1e85b4a14a9cb4071424a4153\clipboard.png)

### 五十四、synchronized的应用方式
...
![img](C:\Users\Administrator\AppData\Local\YNote\data\qq432337FEDCB6EB5443353EB50AE37219\e0bd96ed0bb64ac1ab4403de18967d17\gradle打包流程.png)
...
![image-20191217164659210](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20191217164659210.png)
...
### 六十二、 Java内存结构（java运行时数据区域）

![image-20191219105648132](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20191219105648132.png)

![](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20191217141932232.png)

### 二十三、jvm程序运行时，有六个地方都可以保存数据：
...
![image-20191217142937710](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20191217142937710.png)
...
![image-20191217172607355](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20191217172607355.png)
...
### 七十、容器（HashMap、HashSet、LinkedList、ArrayList、数组等）

​	**HashMap**：包含**Node<K,V>[] table、int size、Set<Map.Entry<K,V>> entrySet**。节点为Node的数组，Node节点包含hash, key,  value, Node<K,V> next。哈希算法计算key的哈希值存放到相应位置。扩容系数是0.75，每次扩容到2倍（扩容后重新排列）。数据结构是数组与链表即ArrayList与LinkedList。
...
![image-20191220172324398](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20191220172324398.png)
...
​	1）线程切换原理通过handle实现，主线程通过Looper.getMainLooper()得到。
