...
### 二十七、rxjava 内存泄漏 （AutoDispose、RxLifecycle解决内存泄漏）
...
1.类型擦除：泛型这种语法糖，编译器会在编译期间「擦除」泛型语法并相应的做出一些类型转换动作。
...
### 四十八、插件化开发（包建强《任玉刚that和张勇》）(Apollo1.2资源冲突解决)
...
**原子性：**由Java内存模型来直接保证原子性的变量操作包括read、load、use、assign、store、write这6个动作。
...
链接：https://www.jianshu.com/p/deb1c62bce5f
